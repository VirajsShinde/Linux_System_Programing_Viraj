...
//Project Name : Customised Virtual File System.(Technology : C, C++ Programming)
/*
 Project By : Viraj Sunil Shinde
...
PINODE Get_Inode(char * name) 
{
	...
	while(temp != NULL)    //Duplicate file name
	{
...
void CreateDILB()    //To inode create
{
	int i = 1;
	PINODE newn = NULL;
...
int CreateFile(char *name,int permission) 
{
	...
	while(temp != NULL)
	{
	 if(temp -> FileType == 0)
		...
		temp = temp -> next;  //temp pointing to vanccant inode
	} 
...
void ls_file() 
{
	...
	while(temp != NULL)
	{
		if(temp -> FileType != 0)  //maxinode - freeinode
		{
